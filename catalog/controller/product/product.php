...
class ControllerProductProduct extends Controller {
	...
	public function index() {
		...
		if ($product_info) {
			...
			if ($product_info['image']) {
				$data['thumb'] = $this->model_tool_image->resize($product_info['image'], 400, 270);
				$this->document->setOgImage($data['thumb']);
			} else {
				$data['thumb'] = $this->model_tool_image->resize('no_image.png', 400, 270);
			}
			...
			if ($product_info['currency_code'] == 'RUB') {
				if (($this->config->get('config_customer_price') && $this->customer->isLogged()) || !$this->config->get('config_customer_price')) {
					$data['price'] = $this->currency->format_RUB($this->tax->calculate($product_info['price'], $product_info['tax_class_id'], $this->config->get('config_tax')));
				} else {
					$data['price'] = false;
				}

				if (($this->config->get('config_customer_price') && $this->customer->isLogged()) || !$this->config->get('config_customer_price')) {
					$data['price2'] = $this->currency->format_RUB($this->tax->calculate($product_info['price2'], $product_info['tax_class_id'], $this->config->get('config_tax')));
				} else {
					$data['price2'] = false;
				}
			}
			...
			if ($product_info['currency_code'] == 'RUB') {
				$data['price_number'] = ($this->tax->calculate($product_info['price'], $product_info['tax_class_id'], $this->config->get('config_tax')));
			} else {
			...
			$currentCat = $this->model_catalog_category->getCategoryName($product_id);
			...
			if($category_id == 92 or $category_info['parent_id']==92) {
				if ($product_info['image']) {
					...
					$data['thumb'] = $this->model_tool_image->resize('no_image.png', 840, 472);
				}
				$data['blogs'] = array();
				...
				if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/product/product_blog.tpl')) {
					$this->response->setOutput($this->load->view($this->config->get('config_template') . '/template/product/product_blog.tpl', $data));
				} else {
					$this->response->setOutput($this->load->view('default/template/product/product_blog.tpl', $data));
				}
			}
			else {
