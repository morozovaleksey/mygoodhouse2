...
class ControllerModuleViewed extends Controller {
	public function index($setting) {
        ...
        if (isset($this->request->get['route']) && $this->request->get['route'] == 'product/product') {
			...
			$category_info = $this->model_catalog_category->getCategory($currentCat['category_id']);

			if($category_info['category_id'] != 92 and $category_info['parent_id'] != 92) {
		...
		foreach ($products as $product_id) {
			...
			if ($product_info) {
				if ($product_info['image']) {
					$image = $this->model_tool_image->resize($product_info['image'], 70, 47);
				} else {
